...
using MooMed.Caching.Cache.UnderlyingCache.Locking.Redis;

namespace MooMed.Caching.Cache.UnderlyingCache
{
	public class RedisCacheImplementation<TKey, TValue> : AbstractCacheImplementation<TKey, TValue>
		where TKey : notnull
	{
		...
		public RedisCacheImplementation(
		...
		public override async ValueTask PutItem(TKey key, TValue value, int? secondsToLive = null)
		{
			...
			await db.StringSetAsync(stringKey, valueSerialized);
		}
