...
namespace MooMed.DotNet.Tests.Extensions
{
    ...
    public class AsyncEnumerableExtensionsTests
    {
        ...
        [Test]
        public async Task ParallelAsyncShouldReturnValues()
        {
            var parallelWorkItems = Enumerable
                .Range(0, 25)
                .ToList();

            var result = await parallelWorkItems.ParallelAsync(Task.FromResult, 5);

            Assert.IsTrue(parallelWorkItems.SequenceEqual(result));
        }
    }
}